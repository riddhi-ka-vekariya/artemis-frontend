...
async function buildFilmTexture(renderer) {
  ...
  for (let i = 0; i < frameCount; i++) {
    ...
    if (images[i]) {
      ...
      ctx.fillStyle = 'rgba(30,20,10,0.28)'; ctx.fillRect(px, py, pw, ph); ctx.restore()
    } else {
      const grad = ctx.createLinearGradient(x0, py, x0, py + ph)
      grad.addColorStop(0, '#3a2f22'); grad.addColorStop(1, '#0a0908')
      ctx.fillStyle = grad; ctx.fillRect(px, py, pw, ph)
    }
    ctx.fillStyle = 'rgba(239,233,223,0.7)'
...
export default function FilmstripViewer() {
  ...
  useEffect(() => {
    ...
    scene.fog = new THREE.Fog(0x141210, 14, 30)
    ...
    async function main() {
      ...
      const mat = new THREE.MeshStandardMaterial({
        map: filmTexture, metalness: 0.6, roughness: 0.3, side: THREE.DoubleSide,
        emissive: 0x4a3a22, emissiveMap: filmTexture, emissiveIntensity: 0.8,
      })
      ...
      reflection.scale.y = -1; reflection.position.y = -2.3
      reflection.rotation.set(-ROT_VAL[0], ROT_VAL[1], ROT_VAL[2])
      scene.add(reflection)

      scene.add(new THREE.AmbientLight(0x5a4a38, 1.4))

      const key = new THREE.SpotLight(0xfff2d8, 6.5, 30, Math.PI / 4, 0.5, 1.0)
      key.position.set(4, 6, 6); key.target.position.set(0, 0, 0)
      scene.add(key, key.target)

      const rim = new THREE.PointLight(0xd7a94a, 3.2, 16)
      rim.position.set(-3, 1, -3); rim._isRim = true; scene.add(rim)

      const fill = new THREE.PointLight(0x8ea0c0, 1.6, 14)
      fill.position.set(-2, -1, 4); scene.add(fill)

      const front = new THREE.PointLight(0xfff6e6, 2.2, 16)
